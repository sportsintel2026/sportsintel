...
async function getBatterVsPitcherHistory(batterId, pitcherId) {
  ...
  try {
    ...
    for (const sp of splits) {
      const s = sp.stat || {};
      totals.atBats += parseIntSafe(s.atBats);
      totals.hits += parseIntSafe(s.hits);
      totals.homeRuns += parseIntSafe(s.homeRuns);
      totals.doubles += parseIntSafe(s.doubles);
      totals.triples += parseIntSafe(s.triples);
      totals.rbi += parseIntSafe(s.rbi);
      totals.walks += parseIntSafe(s.baseOnBalls);
      totals.strikeouts += parseIntSafe(s.strikeOuts);
    }
    ...
    const starts = splits
      ...
      .map(sp => {
        ...
        return {
          ...
          er: parseIntSafe(s.earnedRuns),
          h: parseIntSafe(s.hits),
          k: parseIntSafe(s.strikeOuts),
          bb: parseIntSafe(s.baseOnBalls),
