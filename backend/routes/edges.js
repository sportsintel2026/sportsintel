...
router.get("/mlb", gatePicks, async (req, res) => {
  ...
  try {
    ...
    // WZ-CLEANUP-WINNERBOARD-2026-07-06 :: ?winnerboard preview diagnostic removed -- the winner-first board is live.
    ...
    for (const ge of gameEdges) {
      ...
      moneylineBoard.push({
        ...
        book: pickHome ? ml.homeBook : ml.awayBook,
        opposingBook: pickHome ? ml.awayBook : ml.homeBook,
    ...
    for (const ge of gameEdges) {
      ...
      if (ge.totals.overEdge != null) {
        totalsEdges.push({
          ...
          book: ge.totals.overBook,
          opposingBook: ge.totals.underBook,
      ...
      if (ge.totals.underEdge != null) {
        totalsEdges.push({
          ...
          book: ge.totals.underBook,
          opposingBook: ge.totals.overBook,
  ...
  try {
    ...
    for (const ge of gameEdges) {
      ...
      out.push({
        ...
        time: ge.game.time,
        // Presentation-only verified context. Every value below is already part
        // of this request's calculateGameEdges() snapshot; no provider call is
        // added and none of it feeds back into the model or selection path.
        intel: {
          venue: ge.game.venue ?? null,
          parkRunFactor: ge.game.parkRunFactor ?? null,
          weather: ge.weather ?? null,
          pitchers: {
            away: ge.pitchers?.away ? {
              name: ge.pitchers.away.name ?? null,
              hand: ge.pitchers.away.hand ?? null,
              era: ge.pitchers.away.stats?.era ?? null,
              whip: ge.pitchers.away.stats?.whip ?? null,
              recentStarts: Array.isArray(ge.pitchers.away.recentStarts) ? ge.pitchers.away.recentStarts.length : null,
            } : null,
            home: ge.pitchers?.home ? {
              name: ge.pitchers.home.name ?? null,
              hand: ge.pitchers.home.hand ?? null,
              era: ge.pitchers.home.stats?.era ?? null,
              whip: ge.pitchers.home.stats?.whip ?? null,
              recentStarts: Array.isArray(ge.pitchers.home.recentStarts) ? ge.pitchers.home.recentStarts.length : null,
            } : null,
          },
          bullpen: ge.bullpen ?? null,
          lineups: {
            away: ge.game.lineups?.away ? {
              source: ge.game.lineups.away.source ?? null,
              ops: ge.game.lineups.away.ops ?? null,
              batters: ge.game.lineups.away.batters ?? null,
            } : null,
            home: ge.game.lineups?.home ? {
              source: ge.game.lineups.home.source ?? null,
              ops: ge.game.lineups.home.ops ?? null,
              batters: ge.game.lineups.home.batters ?? null,
            } : null,
          },
        },
        win: mr.win ? {
...
router.get("/nfl", gatePicks, async (req, res) => {
  try {
    ...
    res.json({
      ...
      games: allGames,
      moneylineEdges: moneylineEdges.map((row) => ({ ...row, teamIdentity: allGames.find((game) => String(game.eventId) === String(row.gameId))?.teamIdentity || null })),
      spreadEdges: spreadEdges.map((row) => ({ ...row, teamIdentity: allGames.find((game) => String(game.eventId) === String(row.gameId))?.teamIdentity || null })),
      totalsEdges: totalsEdges.map((row) => ({ ...row, teamIdentity: allGames.find((game) => String(game.eventId) === String(row.gameId))?.teamIdentity || null })),
