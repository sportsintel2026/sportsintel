...
function ConversionCta({ heading, primaryLabel = "Unlock Full WizePicks" }) {
  return <aside className="p2-cta" aria-label="Unlock WizePicks">
    <div><span>READY FOR THE FULL BOARD?</span><h2>{heading}</h2><p>Create an account to access subscriber picks and model details under the existing WizePicks access rules.</p></div>
    <div className="p2-cta-actions"><Link className="p2-cta-primary" to="/signup">{primaryLabel}</Link><Link to="/login">Member log in</Link></div>
  </aside>;
}
...
export function SeoMatchupPage({ sport }) {
  ...
  return <PageFrame page={page}>
    ...
    {state.loading ? <div className="p2-state">Loading the current matchup…</div> : state.error ? <div className="p2-state">The live matchup feed is temporarily unavailable.</div> : !matchedGame ? <div className="p2-state">This matchup is not on the current active WizePicks slate, so this URL is not indexed.</div> : <>
      ...
      <ConversionCta heading={`See every qualified ${SPORT_NAME[sport]} edge in one place`} primaryLabel="See Full Model Board" />
...
export function SeoSlatePage({ sport }) {
  ...
  return <PageFrame page={safePage}>
    ...
    {games.length > 0 && <ConversionCta heading={`Turn this ${sport === "mlb" ? "daily" : "weekly"} slate into your full ${SPORT_NAME[sport]} model board`} primaryLabel="Unlock Full WizePicks" />}
...
export function SeoPerformancePage({ sport, league }) {
  ...
  return <PageFrame page={safePage}>
    ...
    {state.loading ? <div className="p2-state">Loading graded performance…</div> : state.error ? <div className="p2-state">Authoritative performance data is temporarily unavailable.</div> : !season || !state.data?.totalGraded ? <div className="p2-state">No useful authoritative graded sample is available for this sport, so this page should not be indexed.</div> : <>
      ...
      <ConversionCta heading={`Follow the current ${SPORT_NAME[sport]} board behind this verified record`} primaryLabel="Sign Up to Unlock Picks" />
