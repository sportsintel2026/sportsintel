...
export default function HomePage(){
  ...
  const spAdj=[...(e.runLineEdges||[]),...(e.spreadEdges||[])].map(moveAdjust);/* WZ-SPREADFIELD-FIX-2026-07-05 :: football emits spreadEdges; runLineEdges is MLB-only. Include BOTH (exactly one is populated per sport) so the Spread tab fills for NFL/CFB/NBA. */
  ...
  const lineSeries={};
  [...(e.moneylineEdges||[]),...(e.totalsEdges||[]),...(e.runLineEdges||[]),...(e.spreadEdges||[])].forEach(x=>{ const s=seriesFor(x); if(s&&s.length>1) lineSeries[x.gameId+x.side]=s.map(p=>p.o).filter(o=>o!=null); });
  ...
  // WZ-EDGETICKER-SPEED-2026-07-05 :: scroll duration scales with content width so the tape holds a steady, calm pace no matter how wide the items are (long news headlines were whipping past at the old fixed 24s). ~48px/s target, clamped 24-80s.
  const tapeDur = (()=>{ if(!tickerLoop.length) return 24; let chars=0; for(const s of tickerLoop){ chars += (s.kind==="score") ? ((s.a||"").length+(s.h||"").length+String(s.as==null?"":s.as).length+String(s.hs==null?"":s.hs).length+(s.state||"").length+4) : (5+(s.name||"").length+(s.text||"").length); } const px=chars*6.9 + tickerLoop.length*24; return Math.max(24, Math.min(80, Math.round(px/48))); })();
...
.bseg{display:flex;gap:3px;background:#101317;border:1px solid var(--line);border-radius:14px 14px 0 0;padding:4px;margin:10px 4px 0} /* WZ-WIZEBOARD-INSET-2026-07-08 :: tabs inset, rounded top -- board card header */
...
.grid{margin:0 4px;background:var(--panel);border-left:1px solid var(--line);border-right:1px solid var(--line);border-radius:0;overflow:hidden} /* WZ-WIZEBOARD-INSET-2026-07-08 :: rows inset, side borders continue the card */
...
.sum{display:flex;align-items:center;gap:12px;margin:0 4px;padding:11px 14px;font-family:var(--mono);font-size:11px;color:var(--mut);border-top:1px solid var(--line2);border-left:1px solid var(--line);border-right:1px solid var(--line);border-bottom:1px solid var(--line);border-radius:0 0 14px 14px}.sum .l{color:#cfd7e2;font-weight:600}.sum .p{color:var(--green)}.sum .sp{flex:1}
