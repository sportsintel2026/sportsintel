  ...
  if (compact) {
    return <section className="fbintel fbintel--compact" aria-label={`${sport.toUpperCase()} game intel`}>
      {shown.length === 0 ? <div className="fbintel__empty">Verified game intel appears with the selected event-day slate.</div>
        : <div className="fbintel__grid">{shown.map((row) => <IntelCard row={row} key={row.gameId} includeSignal />)}</div>}
    </section>;
  }
    ...
    {shown.length === 0 ? <div className="fbintel__empty">Verified game intel appears with the selected event-day slate.</div> : <>
      <article className={`fbintel__sharp${lead?.marketOnly ? " fbintel__sharp--market" : ""}`}>
        ...
        <div className="fbintel__sharpbody">
          <div><small>{lead?.matchup}</small><h2>{lead?.modelSignal?.pick || "Market information only"}</h2><p>{lead?.modelSignal ? `${lead.modelSignal.market} · verified selected side` : "No independent WizePicks model edge is claimed for this matchup."}</p></div>
      ...
      <div className="fbintel__grid">{shown.map((row) => <IntelCard row={row} key={row.gameId} />)}</div>
    </>}
  </section>;
}

function IntelCard({ row, includeSignal = false }) {
  const factors = factorRows(row);
  return <article className="fbintel__card">
    <div className="fbintel__match"><div><span>{formatEventDate(row.commenceTime ? new Date(row.commenceTime).toLocaleDateString("en-CA", { timeZone: "America/New_York" }) : null, { compact: true })}</span><b>{row.matchup}</b></div><em>{row.dataQuality === "rated" ? "RATED" : "MARKET"}</em></div>
    ...
    {factors.length ? <div className="fbintel__factors">{factors.map(([label, value]) => <div key={label}><span>{label}</span><b>{value}</b></div>)}</div>
